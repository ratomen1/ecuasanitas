SELECT DISTINCT d.afiliacion_id,
                c.numero                                                                                    AS contrato,
                f.numero                                                                                    AS familia,
                A.numero                                                                                    AS afiliado,
                cn.tipoPlan,
                (SELECT obtener_edad(e.fechanacimiento, '2025-12-01'))                                      AS edad,
                s.codigo,
                e.discapacitado,
                a.antiguedad_id,
                ...
                (CASE WHEN n.nombre ILIKE '%POOL%' OR n.nombreplan ILIKE '%POOL%' THEN TRUE ELSE FALSE END) AS espool,
                p.id                                                                                        AS plannivelid,
                ...
                (SELECT STRING_AGG(tiposervicio, ',')
                 FROM promocionweb p
                 WHERE '2025-04-01' BETWEEN p.desde AND p.hasta)                                            AS servicioPromocion,
                c.id                                                                                        AS contratoId,
                o.id,
                e.nombre,
                c.vendedor_id,
                ...
                (CASE WHEN s.codigo = 'VN' THEN o.pagosadelantados ELSE NULL END)                           AS pagosadelantados,
                r.ingreso,
                r.egreso,
                ...
                (CASE WHEN st.id IS NULL THEN FALSE ELSE TRUE END)                                          AS tieneSolicitudTraspaso,
                CAST(DATE_PART('year',
                               AGE(ar.fechaexclusion, ar.fechainicioafiliacion)) AS INTEGER)                AS aniosantiguedad,
                CAST(DATE_PART('day', (NOW() - (ar.fechapago + INTERVAL '1 month'))) AS INTEGER)            AS diasimpago,
                (CASE WHEN a.esreingreso IS NULL THEN FALSE ELSE a.esreingreso END)                         AS esReingreso,
                ...
                (CASE
                     ...
                     ELSE rm.presentadeclaracion END)                                                       AS presentadeclaracion,
                sepagacomisionporreingreso(s.codigo, CAST(DATE_PART('year',
                                                                    AGE(ar.fechaexclusion, ar.fechainicioafiliacion)) AS INTEGER),
...
FROM obligacion o
         ...
         LEFT JOIN solicitudtraspaso st ON st.contratonuevoid = o.contrato_id AND st.fecha = '2025-12-01' AND
                                           st.numerosolicitud = o.numerosolicitud
         ...
         LEFT JOIN LATERAL ( SELECT aa.estadoafiliacion, aa.fechaexclusion, aa.contrato_id, aa.afiliado_id
                             FROM afiliacion aa
                             WHERE aa.contrato_id = a.antiguedad_id
                               AND aa.afiliado_id = a.afiliado_id
                               AND aa.estadoafiliacion IN ('ACT', 'EXC')
                             LIMIT 1 ) aa ON aa.contrato_id = a.antiguedad_id AND aa.afiliado_id = a.afiliado_id
         ...
         LEFT JOIN LATERAL ( SELECT ar.afiliado_id,
                                    ar.contrato_id,
                                    ca.fechapago,
                                    ca.fechainicio,
                                    ...
                                    ar.id,
                                    ROW_NUMBER()
                                    OVER (PARTITION BY ar.afiliado_id ORDER BY ca.fechapago DESC)              AS ROW_NUMBER
                             FROM afiliacion ar
                                      LEFT JOIN contrato ca ON ca.id = ar.contrato_id
                             WHERE ar.estadoafiliacion = 'EXC' ) ar
                   ON ar.afiliado_id = a.afiliado_id AND ar.ROW_NUMBER = 1 AND ar.contrato_id <> a.contrato_id
         LEFT JOIN nivelesnoaplica nna ON CAST(nna.nivel AS int) = n.id
         LEFT JOIN (SELECT c.estado,
                           c.nivel_id,
                           c.fechainicio,
                           ROW_NUMBER() OVER (PARTITION BY c.nivel_id ORDER BY c.fechainicio ASC) AS ROW_NUMBER
                    FROM contrato c
                    WHERE c.estado IN ('ACT', 'ANU')) AS con ON con.nivel_id = n.id AND con.ROW_NUMBER = 1
         ...
         LEFT JOIN (SELECT * FROM calculo_reingreso('2025-12-01')) r ON r.contrato = c.numero AND r.familia =
                                                                                                  (CASE WHEN c.tipoContrato = 'C' THEN f.numero ELSE 0 END) AND
                                                                        r.afiliacion = a.numero
...
WHERE o.fechapagocomision = '2025-12-01'
