SELECT  
                        ...
                        (ar.fechaexclusion-ar.fechainicioafiliacion) AS aniosantiguedad, 
                    //    CAST(DATE_PART('year',AGE(ar.fechaexclusion,ar.fechainicioafiliacion)) AS INTEGER) AS aniosantiguedad, 
                        ...
                        ar.fechaexclusion, 
                        (CASE WHEN rm.presentadeclaracion IS NULL THEN FALSE ELSE rm.presentadeclaracion END) AS presentadeclaracion, 
                        o.comisionista_id AS obligacionComisionistaId, 
                        o.asesorcomericial_id AS obligacionAsesorcomericialId, 
                        coberturaSinComision.datoscoberturas AS coberturassincomision, 
                        (CASE WHEN n.compartir_en_sala_supervisor IS NULL THEN FALSE ELSE n.compartir_en_sala_supervisor END ) AS compartir_en_sala_supervisor 
                    FROM  
                    	obligacion o 
                    ...
                    + 				aa.afiliado_id 
                    + 			FROM afiliacion aa 
                    + 			WHERE aa.contrato_id = a.antiguedad_id AND aa.afiliado_id = a.afiliado_id AND aa.estadoafiliacion IN ('ACT','EXC') LIMIT 1 
                    ...
                    + 			WHERE 
                    + 			c.estado IN ('ACT','ANU') 
                    + 		) AS con ON con.nivel_id = n.id AND con.ROW_NUMBER = 1 
                    	...
                    	LEFT JOIN (SELECT * FROM calculo_reingreso('2024-11-01')) 
                             r ON r.contrato = c.numero and r.familia = (CASE WHEN c.tipoContrato = 'C' THEN f.numero ELSE 0 END) and r.afiliacion = a.numero 
                        LEFT JOIN LATERAL ( 
                           	SELECT 
                    + 				dtl.afiliacion_id , 
                    + 				array_to_string(array_agg('[' || svc.codigo || ',' || dtl.valorcomision ||',' || dtl.id || ']'),',') AS datoscoberturas 
                    + 			FROM obligacion obli 
                    + 			LEFT JOIN detalle dtl ON dtl.obligacion_id = obli.id AND dtl.afiliacion_id = d.afiliacion_id AND dtl.obligacion_id = o.id 
                    + 			LEFT JOIN servicio svc ON svc.id = dtl.servicio_id 
                    + 			WHERE 
                    + 				obli.fechapagocomision = '2024-11-01' 
                    + 				AND dtl.afiliacion_id = d.afiliacion_id 
                    + 				AND dtl.obligacion_id = o.id 
                    + 				AND dtl.codigogrupo = 'COB' 
                    + 				AND obli.servicio_id = 76 
                    + 				AND dtl.valorcomision > 0 
                    + 				GROUP BY dtl.afiliacion_id 
                         ) AS coberturaSinComision ON coberturaSinComision.afiliacion_id = d.afiliacion_id 
                    WHERE 
